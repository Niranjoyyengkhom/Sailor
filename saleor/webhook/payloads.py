...
def serialize_product_channel_listing_payload(channel_listings):
    ...
    channel_listing_payload = serializer.serialize(
        channel_listings,
        fields=fields,
        additional_fields={"channel_slug": (lambda pch: pch.channel, ("slug"))},
    )
...
def generate_product_variant_listings_payload(variant_channel_listings):
    ...
    channel_listing_payload = serializer.serialize(
        ...
        fields=fields,
        additional_fields={"channel_slug": (lambda vch: vch.channel, ("slug"))},
    )
    return channel_listing_payload
...
def generate_product_variant_payload(product_variants: Iterable["ProductVariant"]):
    ...
    payload = serializer.serialize(
        ...
        extra_dict_data={
            ...
            "product_id": lambda v: graphene.Node.to_global_id("Product", v.product.id),
            "media": lambda x: generate_product_variant_media_payload(x),
            "channel_listings": lambda x: json.loads(
                generate_product_variant_listings_payload(x.channel_listings.all())
            ),
        },
    )
    return payload
